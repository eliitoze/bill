...
self.addEventListener('notificationclick', e => {
  ...
  // Bill ni link hoy to NAVI window ma bill kholo.
  // (Pehla app khulu hoy to fakt focus thato hato ane bill dekhato j nahi.)
  const isBillLink = url.indexOf('http') === 0;

  e.waitUntil(
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clientList => {
      if (isBillLink) {
        // Bill pehla thi kya khulu hoy to tya j jao
